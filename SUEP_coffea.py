	...
	def clusterizeTracks(self, events, tracks):
		...
		if not smallEvents:
			...
			for i in range(ncopies):
				tracks = ak.concatenate([tracks, oldtracks], axis=0) # I.e. duplicate our events until we are feeding 5000 events to the clusterizer
			jetdef = fastjet.JetDefinition(fastjet.antikt_algorithm, 1.5)
			cluster = fastjet.ClusterSequence(tracks, jetdef)
			ak15_jets	= ak.with_name(cluster.inclusive_jets(min_pt=0),"Momentum4D")	# These are the ak15_jets
			ak15_consts	= ak.with_name(cluster.constituents(min_pt=0),"Momentum4D")		# And these are the collections of constituents of the ak15_jets
			...
			tracks.fromSUEP = oldtracks.fromSUEP #Otherwise the fromSUEP information gets reset
		...
		if self.doTracksGenMatching:
			...
			fromSUEP_temp = ak.unflatten(ak15_consts.fromSUEP, ak.flatten(shape), axis=1)
	...
	# Sorting jets by pt.
		...
		if self.doTracks:
			...
			if self.doClusters:
				...
				self.constituents.fromSUEP = fromSUEP_temp[highpt_clusters]
